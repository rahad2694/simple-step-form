...
const StepForm = () => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            message: Yup.string()
                ...
                .matches(/[0-9]/, 'It Should contain at least one digit')
                .required('Required'),
        }),
        onSubmit: values => {
    ...
    console.log('Errro er man', isError);
    return (
        <div className="p-5 mt-10">
            ...
            <div className="mt-8 p-4">
                <form className='w-2/4 mx-auto my-5' onSubmit={formik.handleSubmit}>
                    <h1 className='text-3xl text-center my-5 text-green-600'>{currentStep === 5 ? 'Successfully Submitted !' : 'Lets Use FORMIK with YUP'}</h1>
                    {currentStep === 1 && <InputField setIsError={setIsError} formik={formik} itemName="firstName" />}
                    {currentStep === 2 && <InputField setIsError={setIsError} formik={formik} itemName="lastName" />}
                    {currentStep === 3 && <InputField setIsError={setIsError} formik={formik} itemName="email" type="email" />}
                    {currentStep === 4 && <InputField setIsError={setIsError} formik={formik} itemName="message" />}
